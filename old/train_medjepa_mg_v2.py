#!/usr/bin/env python3
"""
MedJEPA LeJEPA training script v2.
...
No split creation and no online probe. Includes final collapsed-BI-RADS probe, PCA 2D/3D, v2 augmentations, timing summary, and optional DataParallel multi-GPU.
...
class ViTEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        proj = self.proj(emb).reshape(b, v, -1).transpose(0, 1)  # [V,B,D]
        return emb, proj
...
class SIGReg(nn.Module):
    ...
    def forward(self, proj: torch.Tensor) -> torch.Tensor:
        ...
        x_t = (proj @ A).unsqueeze(-1) * self.t.to(proj.device, dtype=proj.dtype)
        err = (x_t.cos().mean(dim=-3) - self.phi.to(proj.device, dtype=proj.dtype)).square()
        err = err + x_t.sin().mean(dim=-3).square()
        statistic = (err @ self.weights.to(proj.device, dtype=proj.dtype)) * proj.size(-2)
...
def train_lejepa(net: nn.Module, loss_fn: nn.Module, loader: DataLoader, optimizer, scheduler,
    ...
    history = {"lejepa": [], "invariance": [], "sigreg": [], "proj_std": [], "proj_norm": [], "emb_norm": [], "lr": [], "epoch_time_sec": []}
    ...
    for epoch in range(cfg.epochs):
        ...
        for views, _ in pbar:
            ...
            with torch.no_grad():
                proj_flat = proj.transpose(0, 1).reshape(-1, cfg.projection_dim)
        ...
        print(
            ...
            f"LR {history['lr'][-1]:.2e} | Time {history['epoch_time_sec'][-1]/60:.2f} min",
            flush=True,
        )
