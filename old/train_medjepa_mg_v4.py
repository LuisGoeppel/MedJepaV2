#!/usr/bin/env python3
"""
MedJEPA LeJEPA training script v4.
...
from torch.utils.data import DataLoader, Dataset, TensorDataset, WeightedRandomSampler, DistributedSampler
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--resume-checkpoint", type=str, default="",
                   help="Resume model, optimizer, scheduler, and history from a checkpoint saved by this script.")
...
def build_optimizer_scheduler(net: nn.Module, loader: DataLoader, cfg: TrainConfig):
    ...
    warmup_steps = max(1, optimizer_steps_per_epoch * cfg.warmup_epochs)
...
def train_lejepa(net: nn.Module, loss_fn: nn.Module, loader: DataLoader, optimizer, scheduler,
    ...
    for epoch in range(start_epoch, cfg.epochs):
        ...
        metric_sums = torch.zeros(6, device=device, dtype=torch.float64)
        # [lejepa, invariance, sigreg, proj_std, proj_norm, emb_norm]
        ...
        for batch_idx in pbar:
            ...
            with torch.no_grad():
                ...
                if (batch_idx % diagnostic_every == 0) or (batch_idx + 1 == len(loader)):
                    proj_flat = proj.detach().float().reshape(-1, cfg.projection_dim)
                    metric_sums[3] += proj_flat.std(dim=0).mean().double()
                    metric_sums[4] += proj_flat.norm(dim=1).mean().double()
                    metric_sums[5] += emb.detach().float().norm(dim=1).mean().double()
                    count_tensor[1] += 1.0
        ...
        history["proj_std"].append(float(metric_sums[3].item() / global_diag_nb))
        history["proj_norm"].append(float(metric_sums[4].item() / global_diag_nb))
        history["emb_norm"].append(float(metric_sums[5].item() / global_diag_nb))
...
def main() -> None:
    ...
    if distributed and cfg.use_weighted_sampler:
        raise RuntimeError("--batch-weighting is not implemented for DDP in this v4 script. Use unweighted DDP or add a custom distributed weighted sampler.")
    ...
    if distributed:
        ...
        train_sampler = make_weighted_sampler(train_df) if cfg.use_weighted_sampler else None
