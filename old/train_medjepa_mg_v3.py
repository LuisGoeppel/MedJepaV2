#!/usr/bin/env python3
"""
MedJEPA LeJEPA training script v3.
...
No split creation and no online probe. Includes final collapsed-BI-RADS probe, PCA 2D/3D, v2 augmentations, detailed batch/epoch timing summary, PCA, and fixed optional DataParallel multi-GPU.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--batch-weighting", action="store_true", help="Use collapsed-BI-RADS weighted sampler for SSL batches.")
    ...
    p.add_argument("--multi-gpu", type=str, default="auto", choices=["auto", "none", "data_parallel"],
                   help="auto/data_parallel use torch.nn.DataParallel when >1 CUDA GPU is visible. Batch size is global.")
...
def verify_no_patient_leakage(train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame) -> None:
    if "patient" not in train_df.columns:
        print("WARNING: no patient column found; cannot verify patient leakage.")
        return
    ...
    print("Patient leakage check:", overlaps)
...
def load_splits(cfg: TrainConfig) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for name, df in [("train", train_df), ("val", val_df), ("test", test_df)]:
        ...
        print(f"  {name}: rows={len(df):,}, patients={n_pat}, collapsed={df['collapsed_birads'].value_counts().to_dict()}")
...
def build_optimizer_scheduler(net: nn.Module, loader: DataLoader, cfg: TrainConfig):
    ...
    warmup_steps = max(1, len(loader) * cfg.warmup_epochs)
...
def train_lejepa(net: nn.Module, loss_fn: nn.Module, loader: DataLoader, optimizer, scheduler,
                 cfg: TrainConfig, device: torch.device, dirs: dict[str, Path]) -> dict[str, list[float]]:
    use_cuda = device.type == "cuda"
    amp_dtype = torch.bfloat16 if use_cuda else torch.float32
    history = {
        "lejepa": [], "invariance": [], "sigreg": [], "proj_std": [], "proj_norm": [], "emb_norm": [], "lr": [],
        "epoch_time_sec": [],
    ...
    for epoch in range(cfg.epochs):
        ...
        net.train()
        sums = {k: 0.0 for k in ["lejepa", "invariance", "sigreg", "proj_std", "proj_norm", "emb_norm"]}
        timing_sums = {
        ...
        # Manual iteration lets us measure the time spent waiting for the next batch.
        # This wait includes DataLoader worker time, memmap/PFSS I/O, normalization,
        # foreground crop, all augmentations, collation, and main-process queue wait.
        ...
        for _ in pbar:
            ...
            with torch.no_grad():
                # proj is [B, V, D] in v3.
                proj_flat = proj.reshape(-1, cfg.projection_dim)
                sums["lejepa"] += float(loss.item())
                sums["invariance"] += float(inv.item())
                sums["sigreg"] += float(sig.item())
                sums["proj_std"] += float(proj_flat.std(dim=0).mean().item())
                sums["proj_norm"] += float(proj_flat.norm(dim=1).mean().item())
                sums["emb_norm"] += float(emb.norm(dim=1).mean().item())
        ...
        for k, v in sums.items():
            history[k].append(v / nb)
        history["lr"].append(float(optimizer.param_groups[0]["lr"]))

        epoch_time = float(time.perf_counter() - epoch_start_time)
        history["epoch_time_sec"].append(epoch_time)
        history["num_batches"].append(int(batch_count))

        for k, v in timing_sums.items():
            v = float(v)
            history[k].append(v)
            history[k.replace("_time_sec", "_time_per_batch_sec")].append(v / nb)
        ...
        print(
            ...
            f"ProjStd {history['proj_std'][-1]:.4f} | EmbNorm {history['emb_norm'][-1]:.2f} | "
            f"LR {history['lr'][-1]:.2e} | Time {epoch_time/60:.2f} min | "
            f"Data/Aug {timing_sums['data_wait_and_augmentation_time_sec']/60:.2f} min | "
            f"Fwd {timing_sums['forward_loss_time_sec']/60:.2f} min | "
            f"Bwd {timing_sums['backward_optimizer_time_sec']/60:.2f} min",
            flush=True,
        )
...
def plot_training_history(history: dict[str, list[float]], dirs: dict[str, Path]) -> None:
    ...
    for i, key in enumerate(["proj_std", "proj_norm", "emb_norm"]):
        plt.subplot(1, 3, i + 1)
        plt.plot(epochs, history[key])
        plt.title(key)
        plt.xlabel("Epoch")
...
def maybe_enable_data_parallel(net: nn.Module, cfg: TrainConfig, device: torch.device) -> nn.Module:
    if device.type != "cuda":
        return net
...
def main() -> None:
    ...
    if cfg.image_size <= 0:
        ...
        print(f"Using image_size from augmentation config: {cfg.image_size}", flush=True)
    else:
        print(f"Using image_size from CLI: {cfg.image_size}", flush=True)
    ...
    if device.type == "cuda":
        ...
        print(f"CUDA memory free/total GiB: {free/1024**3:.2f}/{total/1024**3:.2f}")
    ...
    train_loader = make_loader(train_ds, cfg, cfg.batch_size, shuffle=True, drop_last=True, sampler=sampler)
    ...
    print("Views per sample:", cfg.num_views)
    print("Effective augmented views per step:", cfg.batch_size * cfg.num_views)

    t0 = time.perf_counter()
    net = ViTEncoder(cfg).to(device)
    net = maybe_enable_data_parallel(net, cfg, device)
    ...
    t0 = time.perf_counter()
    history = train_lejepa(net, loss_fn, train_loader, optimizer, scheduler, cfg, device, dirs)
    ...
    torch.save(final_payload, dirs["models"] / "final_lejepa_checkpoint.pt")
    torch.save(model_to_save.state_dict(), dirs["models"] / "final_lejepa_encoder_state_dict.pt")
    timing["model_saving_sec"] = time.perf_counter() - t0

    # One collapsed-BI-RADS linear probe after training.
    ...
    summary = {
        ...
        "linear_probe_collapsed_val_accuracy": probe_result["val"]["accuracy"],
        "linear_probe_collapsed_val_balanced_accuracy": probe_result["val"]["balanced_accuracy"],
        "linear_probe_collapsed_val_macro_f1": probe_result["val"]["macro_f1"],
        "linear_probe_collapsed_test_accuracy": probe_result["test"]["accuracy"],
        "linear_probe_collapsed_test_balanced_accuracy": probe_result["test"]["balanced_accuracy"],
        "linear_probe_collapsed_test_macro_f1": probe_result["test"]["macro_f1"],
        "pca": pca_info,
        "timing": timing,
        "multi_gpu": {
