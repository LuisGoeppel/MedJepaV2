...
class SIGReg(nn.Module):
    """LeJEPA-like SIGReg for batch-first multi-view projections.

    The original LeJEPA implementation appears to use projections shaped
    [V, B, D] and averages over the batch dimension. Our encoder returns
    [B, V, D], so this implementation keeps the batch-first convention but
    regularizes each view separately across the batch, then averages over views.

    In DDP, the small projection tensor is gathered across ranks before SIGReg
    so the statistic is estimated on the global batch. Remote ranks are detached;
    the local rank keeps gradients, and DDP handles gradient averaging.
    """

    def __init__(self, knots: int = 17, num_projections: int = 256, gather_distributed: bool = True):
        super().__init__()
    ...
    def _gather_with_local_grad(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Keep gradients for local samples; remote samples contribute to the
        # distribution estimate but are treated as constants on this rank.
        gathered[rank] = x
    ...
    def forward(self, proj: torch.Tensor) -> torch.Tensor:
        ...
        proj = self._gather_with_local_grad(proj)
        ...
        # [B, V, D] @ [D, P] -> [B, V, P] -> [B, V, P, K]
        x_t = (proj @ A).unsqueeze(-1) * self.t.to(device=proj.device, dtype=proj.dtype)

        # LeJEPA-like semantics: average over the batch dimension per view.
        # err: [V, P, K]
        err = (x_t.cos().mean(dim=0) - self.phi.to(device=proj.device, dtype=proj.dtype)).square()
        err = err + x_t.sin().mean(dim=0).square()

        # integrate over K, scale by B, average over views and projections
        statistic = (err @ self.weights.to(device=proj.device, dtype=proj.dtype)) * B
